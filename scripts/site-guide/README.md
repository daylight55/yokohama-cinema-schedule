...
48秒・900×1200・24fpsの無音の図解動画です。日本語と英語で同じ8場面を説明します。
実際のアカウント情報や映画の宣材画像は使用せず、既存のはまむび！ロゴと図形を使っています。
上映・メンバーの表示は説明用のサンプルです。音声・BGM・第三者のキャラクターは使用しません。

## 再生成

Python 3、Pillow 12.3.0、ffmpeg（libx264）を用意し、リポジトリのルートから実行します。
...
動画はH.264/yuv420p、faststart付き。各ファイルをCloudflare Pagesの25MiB制限内に保ちます。
...
日英の再生・シーク・終了、320×700と390×844の横スクロール、テキスト版と字幕、
