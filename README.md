...
トップバーの JP / EN トグルで言語を切り替えます。選択は認証済みユーザーの
D1 `users.language` に保存し、他端末でも読み込みます。招待画面で登録時の言語を
指定でき、既存ユーザーの再ログインでは保存済み言語を上書きしません。
招待メールにも日本語・英語を選択できます。個人メモやメールアドレスは翻訳しません。

作品一覧のタイトルから `#movie?movie=<既存の作品キー>` へ進むと、今日から6日後までの
上映を日付・映画館ごとに表示します。画面の絞り込み設定に左右されず、直接アクセスや
再読み込みにも対応します。お気に入り等の作品キーと既存のハッシュURLは維持します。

上映スケジュールの作品名からも作品別の週間スケジュールへ直接移動できます。
隣の「…」で作品の設定を開くと、気になる・鑑賞済み・興味なしの選択と
「作品の上映スケジュール」リンクを利用できます。各上映回の公式予約サイトは
独立した「予約 / Book」ボタンで開き、鑑賞予定の追加とは分けています。

### 作品名を調べるAgent

作品名の直訳はしません。収集Workerの定期実行後、Workers AIの
`@cf/meta/llama-3.3-70b-instruct-fp8-fast` が検索・候補取得・公式ページ照合を選択します。
Wikidataを検索し、日本語題/別名、映画分類、英語ラベルと英語Wikipediaの項目名を照合します。
映画以外の項目、対応の曖昧な候補、根拠のないモデル生成タイトルは採用しません。
原題はP1476、公式ページはP856の情報を使います。

公式ページ取得は `worker/src/title-research.ts` の配給会社・映画祭の許可ホストに限定し、
モデルから任意URLを受け取りません。ページの作品見出しを照合し、公式出典と公開DB出典を
区別してD1に保存します。未対応ホストや照合不能の場合は公式確認済みとは表示しません。
新しい公式ホストの追加には、運営者の確認が必要です。未確認の作品は日本公開題を維持します。

1実行につき最大5作品、1作品につき最大5回のインターネットアクセス（自動再試行なし）。
未解決の作品は24時間以上空け、累計5回の調査で停止します。403/429や通信失敗では
その実行の調査を停止し、取得済みの上映や作品名を残します。調査済み結果は再取得しません。
`movie_title_research` の `attempts`、`status`、`source_url`、`source_kind` で確認できます。

migration 0024を先に適用し、AIバインディングを含む収集WorkerとPagesをデプロイします。
...
「気になる」作品を自動共有します。グループ作成や個別の公開設定は不要です。
メンバーで絞り込み、同じ上映回の参加者と予約済み状態、作品ごとの登録者を確認できます。
個人の予定・作品の更新は共有画面の再読み込み時にも反映されます。
公開モード・未認証・無効化されたアカウントは利用できません。
住所、移動経路、非表示作品、カレンダー認証情報などは共有APIに含めません。
...
`#about` にコミック風の図解とオリジナルBGMによる使い方動画を掲載しています。上映検索、作品別の週間上映、予約、
